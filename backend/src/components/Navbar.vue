<template>
    <header class="flex justify-between items-center p-3 h-14 shadow bg-white">
        <button @click="emit('toggle-sidebar')" class="flex items-center justify-center rounded transition-colors w-8 h-8 text-gray-700 hover:bg-gray-700/10">
            <MenuIcon class="w-6"/>
        </button>
        <div class="px-4">
            <Menu as="div" class="relative inline-block text-left">
                <div>
                    <MenuButton class="flex items-center">
                        <img src="@/assets/user1.jpeg" alt="user1" class="rounded-full w-11 mr-2">
...
<script setup>
    ...
    const currentUser = computed(()=> store.state.user.data)//returneaza datele
    function logout(){
